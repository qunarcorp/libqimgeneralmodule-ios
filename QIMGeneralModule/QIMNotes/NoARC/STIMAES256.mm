...
+ (NSString *)encryptForBase64:(NSString *)message password:(NSString *)password {
    ...
    NSData *data = [STIMAES256 encryptForData:message password:password];
    if (data) {
        NSString *base64EncodedString = [NSString stimDB_base64StringFromData:data length:[data length]];
...
+ (NSString *)decryptForBase64:(NSString *)base64EncodedString password:(NSString *)password {
    NSData *data = [NSData stimDB_base64DataFromString:base64EncodedString];
    if (password) {
        return [STIMAES256 decryptForData:data password:password];
    }
    return nil;
}
