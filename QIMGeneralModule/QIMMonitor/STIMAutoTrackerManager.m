...
@interface STIMAutoTrackerManager ()

@property(nonatomic, strong) dispatch_queue_t trackerDataManagerQueue;

@end

@implementation STIMAutoTrackerManager

+ (instancetype)sharedInstance {
    static id _sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[STIMAutoTrackerManager alloc] init];
...
- (void)addACTTrackerDataWithEventId:(NSString *)eventId withDescription:(NSString *)description {
    dispatch_async(self.trackerDataManagerQueue, ^{
        ...
        NSString *infoStr = [[STIMJSONSerializer sharedInstance] serializeObject:info];
        [[STIMAutoTrackerDataManager stIMDB_sharedLogDBInstance] stimDB_insertTraceLogWithType:@"ACT" withSubType:@"click" withReportTime:reportTime withLogInfo:infoStr];
    });
}
...
- (void)addCATTraceData:(NSDictionary *)catDic {
    dispatch_async(self.trackerDataManagerQueue, ^{
        ...
        NSString *infoStr = [[STIMJSONSerializer sharedInstance] serializeObject:info];
        [[STIMAutoTrackerDataManager stIMDB_sharedLogDBInstance] stimDB_insertTraceLogWithType:@"CAT" withSubType:@"http" withReportTime:reportTime withLogInfo:infoStr];
    });
}
