...
#import "STIMCalendarsManager.h"
#import <EventKit/EventKit.h>
#import "STIMPublicRedefineHeader.h"

@interface STIMCalendarsManager ()
...
@implementation STIMCalendarsManager

+ (instancetype)sharedEventCalendar {
    static STIMCalendarsManager *eventCalendar = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        eventCalendar = [[STIMCalendarsManager alloc] init];
...
- (void)checkCalendarCanUsedCompletion:(completion)completion {
    ...
    if (eventStatus == EKAuthorizationStatusAuthorized) {
        ...
        [self.eventStore requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *_Nullable error) {
            ...
            isError = error;
            if (granted) {
                STIMVerboseLog(@"用户点击了允许访问日历");
            } else {
                STIMVerboseLog(@"用户没有点允许访问日历");
            }
        }];
        if (self.completion) {
        ...
        [self.eventStore requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (error) {
                    ...
                    STIMVerboseLog(@"eventId : %@", event.eventIdentifier);
                    if (self.completion) {
