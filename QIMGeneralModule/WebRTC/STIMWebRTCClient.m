...
#import "STIMVideoDecoderFactory.h"
#import "STIMRTCSettingModel.h"


@interface STIMWebRTCClient () <RTCPeerConnectionDelegate, RTCVideoViewDelegate>
...
@implementation STIMWebRTCClient {
    int _webRTCType;
}

static STIMWebRTCClient *instance = nil;

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[STIMWebRTCClient alloc] init];
...
//    NSString *httpUrl = [NSString stringWithFormat:@"https://qim.qunar.com/rtc/index.php?username=%@", [[[STIMKit sharedInstance] thirdpartKeywithValue] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
     NSString *httpUrl = [NSString stringWithFormat:@"%@rtc?username=%@",[[STIMKit sharedInstance] qimNav_VideoUrl] , [[[STIMKit sharedInstance] thirdpartKeywithValue] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
...
//    STIMVideoEncoderFactory * encoderFactory = [[STIMVideoEncoderFactory alloc]init];
//    STIMVideoDecoderFactory * decoderFactory = [[STIMVideoDecoderFactory alloc]init];
    STIMRTCSettingModel * model = [[STIMRTCSettingModel alloc]init];
...
- (void)showRTCViewByXmppId:(NSString *)remoteJid isVideo:(BOOL)isVideo isCaller:(BOOL)isCaller {
    ...
    self.rtcView = [[STIMRTCSingleView alloc] initWithWithXmppId:remoteJid IsVideo:isVideo isCallee:!isCaller];
    [self.rtcView show];
    [self.rtcView updateRemoteUserInfoWithXmppId:remoteJid];
    // 2.播放声音
    NSURL *audioURL;
    if (isCaller) {
        audioURL = [NSURL URLWithString:[NSBundle stimDB_myLibraryResourcePathWithClassName:@"STIMGeneralModule" BundleName:@"STIMWebRTCIcons" pathForResource:@"AVChat_waitingForAnswer" ofType:@"mp3"]];
    } else {
        audioURL = [NSURL URLWithString:[NSBundle stimDB_myLibraryResourcePathWithClassName:@"STIMGeneralModule" BundleName:@"STIMWebRTCIcons" pathForResource:@"AVChat_incoming" ofType:@"mp3"]];
    }
    ...
    if (isVideo) {
        _webRTCType = STIMMessageType_WebRTC_Vedio;
    }
    else
    {
        _webRTCType = STIMMessageType_WebRTC_Audio;
    }
    // 5.做RTC必要设置
    if (isCaller) {
        [self initRTCSetting];
        ...
        NSString *extentInfo = [[STIMJSONSerializer sharedInstance] serializeObject:dict];
        [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"create" WithExtentInfo:extentInfo WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
        ...
        self.timeOutTimer = [NSTimer scheduledTimerWithTimeInterval:1 repeats:YES block:^(NSTimer * _Nonnull timer) {
            if (timerCount<=0) {
                ...
                NSString *extentInfo = [[STIMJSONSerializer sharedInstance] serializeObject:dict];
                [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"timeout" WithExtentInfo:extentInfo WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
//                STIMMessageModel *msg = [[STIMKit sharedInstance] sendMessage:@"对方无人接听" WithInfo:nil ToUserId:self.remoteJID WithMsgType:_webRTCType];
                
                STIMMessageModel *msg = [[STIMKit sharedInstance] sendMessage:[NSBundle stimDB_localizedStringForKey:@"atom_rtc_avcall"] WithInfo:extentInfo ToUserId:self.remoteJID WithMsgType:_webRTCType];
...
//        STIMMessageModel *msg = [[STIMKit sharedInstance] sendMessage:@"[当前客户端不支持音视频]" WithInfo:nil ToUserId:self.remoteJID WithMsgType:STIMMessageType_WebRTC_Vedio];
...
//        });
        ...
        STIMVerboseLog(@"如果是接收者，就要处理信令信息");
...
- (void)listenSystemCall {
    ...
    self.callCenter.callEventHandler = ^(CTCall *call) {
        if ([call.callState isEqualToString:CTCallStateDisconnected]) {
            STIMVerboseLog(@"Call has been disconnected");
        } else if ([call.callState isEqualToString:CTCallStateConnected]) {
            STIMVerboseLog(@"Call has just been connected");
        } else if ([call.callState isEqualToString:CTCallStateIncoming]) {
            STIMVerboseLog(@"Call is incoming");
        } else if ([call.callState isEqualToString:CTCallStateDialing]) {
            STIMVerboseLog(@"call is dialing");
        } else {
            STIMVerboseLog(@"Nothing is done");
        }
    };
}
...
- (RTCSessionDescription *)descriptionWithDescription:(RTCSessionDescription *)description videoFormat:(NSString *)videoFormat {
    ...
    if (mLineIndex == -1) {
        ...
        STIMVerboseLog(@"No m=video line, so can't prefer %@", videoFormat);
        return description;
    }
...
- (NSString *)getLocalSDPByRemoteSDP:(NSString *)sdp {
    ...
    STIMVerboseLog(@"&*******& Remote SDP %@", sdp);
    return sdp;
}

- (NSString *)getRemoteSDPByLocalSDP:(NSString *)sdp {
    STIMVerboseLog(@"&*******& Local SDP %@", sdp);
    sdp = [sdp stringByReplacingOccurrencesOfString:@"\r" withString:@"\\r"];
    sdp = [sdp stringByReplacingOccurrencesOfString:@"\n" withString:@"\\n"];
    return sdp;
}
...
- (void)hangupEvent{
    ...
    NSString *extentInfo = [[STIMJSONSerializer sharedInstance] serializeObject:dict];
//    [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"close" WithExtentInfo:extentInfo WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
...
- (void)receiveSignalingMessage:(NSNotification *)notification {
    ...
    if ([userID isEqualToString:self.remoteJID]) {
        ...
        STIMVerboseLog(@"===========音视频信息=========\r receiveAudioVideoMsgNotify extendInfo %@", extendInfo);
        NSDictionary *infoDic = [[STIMJSONSerializer sharedInstance] deserializeObject:extendInfo error:nil];
...
- (void)acceptAction {
    ...
    NSString *extentInfo = [[STIMJSONSerializer sharedInstance] serializeObject:dict];
    [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"pickup" WithExtentInfo:extentInfo WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
    [self.audioPlayer stop];
    [self initRTCSetting];
    self.callConnected = YES;
    __weak STIMWebRTCClient *weakSelf = self;
    [self.peerConnection offerForConstraints:self.sdpConstraints completionHandler:^(RTCSessionDescription *_Nullable sdp, NSError *_Nullable error) {
        STIMWebRTCClient *strongSelf = weakSelf;
...
//
        ...
        [self.peerConnection setRemoteDescription:remoteSdp completionHandler:^(NSError *_Nullable error) {
            STIMVerboseLog(@"setRemoteSDP : %@", remoteSdp);
        ...
        if (self.peerConnection.remoteDescription) {
            ...
            STIMVerboseLog(@"Add ICE %@", candidate);
        } else {
            STIMVerboseLog(@"");
        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.rtcView.callee && !self.callConnected) {
                ...
                NSString *extentInfo = [[STIMJSONSerializer sharedInstance] serializeObject:tempdict];
                [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"deny" WithExtentInfo:extentInfo WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
//                STIMMessageModel *msg = [[STIMKit sharedInstance] sendMessage:@"已拒绝" WithInfo:extentInfo ToUserId:self.remoteJID WithMsgType:_webRTCType];
//                dispatch_async(dispatch_get_main_queue(), ^{
//                    [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationMessageUpdate object:self.remoteJID userInfo:@{@"message": msg}];
//                });
                ...
                NSString *extentInfo = [[STIMJSONSerializer sharedInstance] serializeObject:tempdict];
                [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"cancel" WithExtentInfo:extentInfo WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
                
                STIMMessageModel *msg = [[STIMKit sharedInstance] sendMessage:[NSBundle stimDB_localizedStringForKey:@"atom_rtc_canceled"] WithInfo:extentInfo ToUserId:self.remoteJID WithMsgType:_webRTCType];
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationMessageUpdate object:self.remoteJID userInfo:@{@"message": msg}];
                });
            }
                ...
                if (!self.rtcView.callee) {
                    ...
                    STIMMessageModel *msg = [[STIMKit sharedInstance] sendMessage:[NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"atom_rtc_duration"],timeStr] WithInfo:extentInfo ToUserId:self.remoteJID WithMsgType:_webRTCType];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationMessageUpdate object:self.remoteJID userInfo:@{@"message": msg}];
                    });
                }
...
//        NSString *extentInfo = [[STIMJSONSerializer sharedInstance] serializeObject:dict];
//        [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"busy" WithExtentInfo:extentInfo WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
...
//        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.rtcView) {
                ...
                STIMMessageModel *msg = [[STIMKit sharedInstance] sendMessage:[NSBundle stimDB_localizedStringForKey:@"atom_rtc_deny_other"] WithInfo:extentInfo ToUserId:self.remoteJID WithMsgType:_webRTCType];
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationMessageUpdate object:self.remoteJID userInfo:@{@"message": msg}];
                });
                [self.rtcView dismiss];
        ...
        NSString *extentInfo = [[STIMJSONSerializer sharedInstance] serializeObject:tempdict];
        [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"cancel" WithExtentInfo:extentInfo WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
//        STIMMessageModel *msg = [[STIMKit sharedInstance] sendMessage:@"对方已取消" WithInfo:extentInfo ToUserId:self.remoteJID WithMsgType:_webRTCType];
//        dispatch_async(dispatch_get_main_queue(), ^{
//            [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationMessageUpdate object:self.remoteJID userInfo:@{@"message": msg}];
//        });
//        if (self.rtcView) {
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection didChangeSignalingState:(RTCSignalingState)stateChanged {
    STIMVerboseLog(@"信令状态改变 Signaling state changed: %ld", (long) stateChanged);
    switch (stateChanged) {
...
didStartReceivingOnTransceiver:(RTCRtpTransceiver *)transceiver {
    ...
    STIMVerboseLog(@"Now receiving %@ on track %@.", track.kind, track.trackId);
}
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection
    ...
    STIMVerboseLog(@"didAddReceiver :");
}

/** Called when the receiver and its track are removed. */
- (void)peerConnection:(RTCPeerConnection *)peerConnection
     didRemoveReceiver:(RTCRtpReceiver *)rtpReceiver {
    STIMVerboseLog(@"didRemoveReceiver :");
}
...
//    STIMVerboseLog(@"peerConnection %@",pee)
    switch (newState) {
        case RTCIceConnectionStateNew: {
            STIMVerboseLog(@"newState = RTCICEConnectionNew");
            [self setConnectLabelText:[NSBundle stimDB_localizedStringForKey:@"Connecting…"]];//@"连接中..."];
        }
            break;
        case RTCIceConnectionStateChecking: {
            STIMVerboseLog(@"newState = RTCICEConnectionChecking");
        }
            break;
        case RTCIceConnectionStateConnected: {
            STIMVerboseLog(@"newState = RTCICEConnectionConnected");//15:56:56.698 15:56:57.570
            [self setConnectLabelText:@"已连接"];
        }
            break;
        case RTCIceConnectionStateCompleted: {
            STIMVerboseLog(@"newState = RTCICEConnectionCompleted");//5:56:57.573
        }
            break;
        case RTCIceConnectionStateFailed: {
            STIMVerboseLog(@"newState = RTCICEConnectionFailed");
            [self setConnectLabelText:@"连接失败..."];
            [self hangupEvent];
        }
            break;
        case RTCIceConnectionStateDisconnected: {
            STIMVerboseLog(@"newState = RTCICEConnectionDisconnected");
            [self setConnectLabelText:@"连接断开..."];
            [self hangupEvent];
        }
            break;
        case RTCIceConnectionStateClosed: {
            STIMVerboseLog(@"newState = RTCICEConnectionClosed");
            [self setConnectLabelText:@"关闭..."];
            [self hangupEvent];
        }
            break;
        case RTCIceConnectionStateCount: {
            STIMVerboseLog(@"newState = RTCICEConnectionMax");
...
didChangeIceGatheringState:(RTCIceGatheringState)newState {
    STIMVerboseLog(@"peerConnection iceGatheringChanged %s", __func__);
    switch (newState) {
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection didAddStream:(RTCMediaStream *)stream {
    STIMVerboseLog(@"Stream with %lu video tracks and %lu audio tracks was added.",
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection didGenerateIceCandidate:(RTCIceCandidate *)candidate {
    ...
    NSString *content = [[STIMJSONSerializer sharedInstance] serializeObject:dic];
    [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:@"candidate" WithExtentInfo:content WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];

    self.HaveSentCandidate = YES;
    STIMVerboseLog(@"新的 Ice candidate 被发现. %@", content);
}

- (void)peerConnection:(RTCPeerConnection *)peerConnection
didRemoveIceCandidates:(NSArray<RTCIceCandidate *> *)candidates {
    STIMVerboseLog(@"移除Ice candidate");
}

// New data channel has been opened.
- (void)peerConnection:(RTCPeerConnection *)peerConnection
    didOpenDataChannel:(RTCDataChannel *)dataChannel {
    STIMVerboseLog(@"New data channel has been opened.");
}
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection didCreateSessionDescription:(RTCSessionDescription *)sdp
                      error:(NSError *)error {
    if (error) {
        STIMVerboseLog(@"创建SessionDescription 失败 : %@", error);
    } else {
        STIMVerboseLog(@"创建SessionDescription 成功");
        ...
        NSString *content = [[STIMJSONSerializer sharedInstance] serializeObject:dic];
        [[STIMKit sharedInstance] sendAudioVideoWithType:_webRTCType WithBody:type WithExtentInfo:content WithMsgId:[STIMUUIDTools UUID] ToJid:[self getRemoteFullJid]];
    }
}
...
- (void)videoView:(RTCEAGLVideoView *)videoView didChangeVideoSize:(CGSize)size {
    if (videoView == self.localVideoView) {
        ...
        STIMVerboseLog(@"remote size === %@", NSStringFromCGSize(size));
        dispatch_async(dispatch_get_main_queue(), ^{
