...
@interface STIMRTCSingleView () <STIMRTCHeaderViewDidClickDelegate>
...
@property(nonatomic, strong) STIMRTCButton *convertAudioBtn;

@property(nonatomic, strong) STIMRTCButton *switchCameraBtn;
...
@implementation STIMRTCSingleView
...
//    [self addSubview:self.otherView];
    ...
    imageView.image = [UIImage imageWithData:[STIMKit defaultUserHeaderImage]];
    [view addSubview:imageView];
    
    UILabel *nameLabel = [[UILabel alloc] init];
    NSString *text = [NSString stringWithFormat:@"%@邀请你音频通话", [[STIMKit sharedInstance] getUserMarkupNameWithUserId:self.remoteJid]];
...
- (void)setUpHeaderView {
    ...
    [self.topUserInfoView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo([[STIMDeviceManager sharedInstance] getNAVIGATION_BAR_HEIGHT]);
...
- (void)initUIForVideoCallee {
    ...
    UILabel *nameLabel = [[UILabel alloc] init];
    NSString *text = [NSString stringWithFormat:@"%@邀请你视频通话", [[STIMKit sharedInstance] getUserMarkupNameWithUserId:self.remoteJid]];
    [nameLabel setText:text];
    [nameLabel setTextAlignment:NSTextAlignmentCenter];
    [view addSubview:nameLabel];
...
- (void)updateRemoteUserInfoWithXmppId:(NSString *)xmppId {
    dispatch_async(dispatch_get_main_queue(), ^{
        self.headerImageView.image = [UIImage imageWithData:[STIMKit defaultUserHeaderImage]];
        self.nameLabel.text = [[STIMKit sharedInstance] getUserMarkupNameWithUserId:xmppId];
    });
}
...
- (STIMRTCButton *)convertAudioBtn {
    if (!_convertAudioBtn) {
        _convertAudioBtn = [[STIMRTCButton alloc] initWithTitle:@"切换语音" noHandleImageName:@"icon_avp_loudspeaker_black"];
...
- (STIMRTCButton *)switchCameraBtn {
    if (!_switchCameraBtn) {
        _switchCameraBtn = [[STIMRTCButton alloc] initWithTitle:@"切换摄像头" noHandleImageName:@"voip_camera_icons_66x66_"];
