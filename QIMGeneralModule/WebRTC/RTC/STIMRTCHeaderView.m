...
@implementation STIMRTCHeaderView
...
- (NSString *)getUserName {
    ...
    NSString *name = nil;
    NSDictionary *userInfo = [[STIMKit sharedInstance] getUserInfoByUserId:_userId];
    //备注
...
- (void)getHeaderImage {
    ...
    if(userInfo){
        ...
        if (urlStr) {
            [self.headerView stimDB_setImageWithURL:[NSURL URLWithString:urlStr] WithChatType:0];
        }
        else{
            NSData *imageData = [STIMKit defaultUserHeaderImage];
...
- (void)clickUserHeaderView {
    if (self.rtcHeaderViewDidClickDelegate && [self.rtcHeaderViewDidClickDelegate respondsToSelector:@selector(didClickUserSTIMRTCHeaderViewWithTag:)]) {
        [self.rtcHeaderViewDidClickDelegate didClickUserSTIMRTCHeaderViewWithTag:self.tag];
    }
}
