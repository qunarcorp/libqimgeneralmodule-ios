...
#import "STIMRTCButton.h"
#import "STIMRTCHeaderView.h"
...
@interface STIMRTCView () <STIMRTCHeaderViewDidClickDelegate>

@property(nonatomic, strong) NSMutableArray *adverseUser;

@property(nonatomic, copy) NSString *hasShowUserName;

@property(nonatomic, strong) STIMRTCViewController *rootRTCViewController;
/** 最小化 */
...
/** 前置、后置摄像头切换按钮 */
@property(strong, nonatomic) STIMRTCButton *swichBtn;
/** 静音按钮 */
@property(strong, nonatomic) STIMRTCButton *muteBtn;
/** 摄像头按钮 */
@property(strong, nonatomic) STIMRTCButton *cameraBtn;
/** 扬声器按钮 */
@property(strong, nonatomic) STIMRTCButton *loudspeakerBtn;
/** 邀请成员按钮 */
@property(strong, nonatomic) STIMRTCButton *inviteBtn;
/** 消息回复按钮 */
@property(strong, nonatomic) UIButton *msgReplyBtn;
/** 收到视频通话时，语音接听按钮 */
@property(strong, nonatomic) STIMRTCButton *voiceAnswerBtn;
/** 挂断按钮 */
@property(strong, nonatomic) STIMRTCButton *hangupBtn;
/** 接听按钮 */
@property(strong, nonatomic) STIMRTCButton *answerBtn;
/** 收起按钮 */
@property(strong, nonatomic) STIMRTCButton *packupBtn;
/** 视频通话缩小后的按钮 */
@property(strong, nonatomic) UIButton *videoMicroBtn;
/** 音频通话缩小后的按钮 */
@property(strong, nonatomic) STIMRTCButton *microBtn;
/** 遮罩视图 */
...
@implementation STIMRTCView {
    int _startHeaderX;
    int _startX;
    int _startY;
}

- (STIMRTCViewController *)rootRTCViewController {
    if (!_rootRTCViewController) {
        _rootRTCViewController = [[STIMRTCViewController alloc] init];
    }
...
//            [[STIMWebRTCClient sharedInstance] resizeViews];
        }];
...
//            [[STIMWebRTCClient sharedInstance] resizeViews];
//            [[STIMWebRTCMeetingClient sharedInstance] answerJoinRoom];
        }];
    } else {
...
- (void)loudspeakerClick {
    STIMVerboseLog(@"外放声音%s", __func__);
...
- (void)inviteClick {
    STIMVerboseLog(@"邀请成员%s", __func__);
...
- (void)msgReplyClick {
    ...
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:nil delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Cancel"] destructiveButtonTitle:nil otherButtonTitles:nil];
...
- (STIMRTCButton *)swichBtn {
    if (!_swichBtn) {
        _swichBtn = [[STIMRTCButton alloc] initWithTitle:nil noHandleImageName:@"icon_avp_camera_white"];
...
- (STIMRTCButton *)muteBtn {
    if (!_muteBtn) {
        _muteBtn = [[STIMRTCButton alloc] initWithTitle:@"静音" imageName:@"icon_avp_mute" isVideo:_isVideo];
...
- (STIMRTCButton *)cameraBtn {
    if (!_cameraBtn) {
        _cameraBtn = [[STIMRTCButton alloc] initWithTitle:@"摄像头" imageName:@"icon_avp_video" isVideo:_isVideo];
...
- (STIMRTCButton *)loudspeakerBtn {
    if (!_loudspeakerBtn) {
        _loudspeakerBtn = [[STIMRTCButton alloc] initWithTitle:@"扬声器" imageName:@"icon_avp_loudspeaker" isVideo:_isVideo];
...
- (STIMRTCButton *)inviteBtn {
    if (!_inviteBtn) {
        _inviteBtn = [[STIMRTCButton alloc] initWithTitle:@"邀请成员" imageName:@"icon_avp_invite" isVideo:_isVideo];
...
- (STIMRTCButton *)hangupBtn {
    if (!_hangupBtn) {
        if (_callee && !_answered) {
            _hangupBtn = [[STIMRTCButton alloc] initWithTitle:@"拒绝" noHandleImageName:@"icon_call_reject_normal"];
        } else if (!_callee && !_answered && _isJoin) {
            _hangupBtn = [[STIMRTCButton alloc] initWithTitle:@"拒绝" noHandleImageName:@"icon_call_reject_normal"];
        } else {
            _hangupBtn = [[STIMRTCButton alloc] initWithTitle:nil noHandleImageName:@"icon_call_reject_normal"];
        }
...
- (STIMRTCButton *)packupBtn {
    if (!_packupBtn) {
        _packupBtn = [[STIMRTCButton alloc] initWithTitle:@"收起" imageName:@"icon_avp_reduce" isVideo:_isVideo];
...
- (UIButton *)msgReplyBtn {
    if (!_msgReplyBtn) {
        if (self.isVideo) {
            _msgReplyBtn = [[STIMRTCButton alloc] initWithTitle:@"消息回复" noHandleImageName:@"icon_av_reply_message_normal"];
        } else {
...
- (STIMRTCButton *)voiceAnswerBtn {
    if (!_voiceAnswerBtn) {
        _voiceAnswerBtn = [[STIMRTCButton alloc] initWithTitle:@"语音接听" noHandleImageName:@"icon_av_audio_receive_normal"];
...
- (STIMRTCButton *)answerBtn {
    if (!_answerBtn) {
        _answerBtn = [[STIMRTCButton alloc] initWithTitle:@"接听" noHandleImageName:@"icon_audio_receive_normal"];
        [_answerBtn addTarget:self action:@selector(answerClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _answerBtn;
}

- (STIMRTCButton *)microBtn {
    if (!_microBtn) {
        _microBtn = [[STIMRTCButton alloc] initWithTitle:@"等待中" noHandleImageName:@"icon_av_audio_micro_normal"];
...
- (void)addRemoteUserHeaderImageViewWithUserName:(NSString *)userName andtag:(NSInteger)tag{
    ...
    STIMRTCHeaderView *headerView = [[STIMRTCHeaderView alloc] initWithinitWithFrame:CGRectMake(_startHeaderX, 0, 65, 65) userId:userId];
...
- (void)didClickUserSTIMRTCHeaderViewWithTag:(NSInteger)tag {
    if (tag >= 0) {
        ...
        if (![userId isEqualToString:self.hasShowUserName]) {
            [[STIMWebRTCMeetingClient sharedInstance] addedStreamWithClickUserId:userId];
        }
    }
}
...
//                [[STIMWebRTCClient sharedInstance] changeViews];
//                [self.adverseStackView removeAllSubviews];
...
- (void)resetRemoteVideoView {
    ...
    if (self.adverseUser.count) {
        [[STIMWebRTCMeetingClient sharedInstance] addedStreamWithClickUserId:[self.adverseUser objectAtIndex:0]];
    } else {
        [self.adverseStackView removeAllSubviews];
    }
    [UIView animateWithDuration:0.25 animations:^{
...
- (void)showAlertMessage:(NSString *)message {
    UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:message preferredStyle:UIAlertControllerStyleAlert];
