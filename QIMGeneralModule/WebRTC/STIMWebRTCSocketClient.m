...
@interface STIMWebRTCSocketClient () <SRWebSocketDelegate> {
...
@implementation STIMWebRTCSocketClient
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _rtcSocketHost = [[STIMKit sharedInstance] qimNav_WssHost];
        _httpRtcSocketHost = [[STIMKit sharedInstance] qimNav_VideoApiHost];
...
//http://150.242.184.16:8080/room/
    NSString *remoteKey = [[STIMKit sharedInstance] thirdpartKeywithValue];
...
//
    NSString * url = [NSString stringWithFormat:@"%@rtc?action=conference&method=get_servers&username=%@",[[STIMKit sharedInstance]qimNav_VideoUrl],remoteKey];
...
- (void)sendPing {
    ...
    NSString *message = [[STIMJSONSerializer sharedInstance] serializeObject:pingDic];
...
- (void)sendMessage:(NSMutableDictionary *)params msgId:(int)msgId complete:(Callback)complete {
    if (params) {
        [params setObject:[[STIMKit sharedInstance] thirdpartKeywithValue] forKey:@"ckey"];
    ...
    NSString *message = [[STIMJSONSerializer sharedInstance] serializeObject:params];
    if ([self.webSocket readyState] == SR_OPEN) {
...
- (void)sendMessage:(NSMutableDictionary *)params WithRoomEvent:(WebRTCRoomEvent)roomEvent WithMsgId:(int)msgId complete:(Callback)complete {
    ...
    NSString *message = [[STIMJSONSerializer sharedInstance] serializeObject:messageDic];
...
- (void)paserMessage:(NSString *)message {
    ...
    NSDictionary *messageDic = [[STIMJSONSerializer sharedInstance] deserializeObject:message error:nil];
...
- (void)joinRoom:(NSString *)roomName WithTopic:(NSString *)topic WithNickName:(NSString *)nickName complete:(void (^)(NSDictionary *))complete {
    ...
    [params setObject:@([[STIMKit sharedInstance] getCurrentServerTime]) forKey:@"startTime"];
...
- (void)sendICECandidateWithEndpointName:(NSString *)endpointName WithCandidate:(NSString *)candidate WithSdpMLineIndex:(int)sdpMLineIndex WithSdpMid:(NSString *)sdpMid complete:(void (^)(BOOL))complete {
    ...
    STIMVerboseLog(@"Send Candidate %@", params);
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket {
    STIMVerboseLog(@"Websocket Connected");
...
- (void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error {
    STIMVerboseLog(@"Websocket Failed With Error %@", error);
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message {
    STIMVerboseLog(@"Received \"%@\"", message);
    [self paserMessage:message];
}

//长链接关闭
- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
    STIMVerboseLog(@"WebSocket closed");
    if ([self.delegate respondsToSelector:@selector(webRTCSocketClient:didCloseWithCode:reason:wasClean:)]) {
...
- (void)webSocket:(SRWebSocket *)webSocket didReceivePong:(NSData *)pongPayload; {
    ...
    STIMVerboseLog(@"WebSocket received pong : %@", reply);
}
