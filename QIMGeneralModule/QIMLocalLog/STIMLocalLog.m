...
@interface STIMLocalLog ()

@end

@implementation STIMLocalLog

+ (void)load {
    [STIMLocalLog sharedInstance];
}

+ (instancetype)sharedInstance {
    static STIMLocalLog *__localLog = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __localLog = [[STIMLocalLog alloc] init];
    });
    return __localLog;
}
...
- (void)startLog {
    ...
    [[STIMKit sharedInstance] setCacheName:[[STIMKit sharedInstance] getLastJid]];
    STIMLocalLogType logType = [[[STIMKit sharedInstance] userObjectForKey:@"recordLogType"] integerValue];
    logType = STIMLocalLogTypeOpened;
    ...
    if ([lastUserName containsString:@"dan.liu"] || [lastUserName containsString:@"weiping.he"] || [lastUserName containsString:@"geng.li"] || [lastUserName containsString:@"lilulucas.li"] || [lastUserName containsString:@"ping.xue"] || [lastUserName containsString:@"wenhui.fan"] || [lastUserName containsString:@"ping.yang"]) {
        ...
        STIMLocalLogType logType = [[[STIMKit sharedInstance] userObjectForKey:@"recordLogType"] integerValue];
        if (logType == STIMLocalLogTypeDefault) {
            [[STIMKit sharedInstance] setUserObject:@(STIMLocalLogTypeOpened) forKey:@"recordLogType"];
        }
    }
    STIMLocalLogType newlogType = [[[STIMKit sharedInstance] userObjectForKey:@"recordLogType"] integerValue];
    if (newlogType == STIMLocalLogTypeOpened) {
        [self deleteLocalLog];
        [self initDDLog];
    }
    [self initDDLog];
...
- (void)initDDLog {
    ...
    STIMLogFormatter *logFormatter = [[STIMLogFormatter alloc] init];
...
- (void)redirectNSLogToDocumentFolder {
    ...
    // 将log输入到文件
    STIMVerboseLog(@"本地日志路径 : %@", logFilePath);
...
- (NSString *)createNewLogFileWithDirectory:(NSString *)logDirectory {
    ...
    NSDateFormatter *dateFormatter = [NSDateFormatter stimDB_defaultDateFormatter];
...
- (void)deleteLocalLog {
    ...
    for (NSString *logFilePath in logArray) {
        ...
        if (fabs(fabs(timeIntervalSinceNow) / (3600 * 24 * 1.5)) >= 1) { //删除间隔超过一天半的日志
            ...
            if (removeSuccess) {
                STIMVerboseLog(@"删除旧日志<%@>成功", logFilePath);
            } else {
                STIMVerboseLog(@"<删除旧日志失败, 失败原因 : %@>", error);
            }
        }
    }
}
...
- (NSData *)allLogData {
    ...
    //缓存Path
    NSString *cachePath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:[NSString stringWithFormat:@"/%@/", [[STIMKit sharedInstance] getLastJid]]];
    [logArray addObject:cachePath];


    //数据库文件
    NSString *dbPath = [[STIMKit sharedInstance] getDBPathWithUserXmppId:[[STIMKit sharedInstance] getLastJid]];
    ...
    NSString *zipFileName = [NSString stringWithFormat:@"%@-log.zip", [[STIMKit sharedInstance] getLastJid]];

    NSString *zipFilePath = [[STIMZipArchive sharedInstance] zipFiles:logArray ToFile:[[STIMLocalLog sharedInstance] getLocalZipLogsPath] ToZipFileName:zipFileName WithZipPassword:@"lilulucas.li"];
...
- (void)submitFeedBackWithContent:(NSString *)content withUserInitiative:(BOOL)initiative {
    STIMVerboseLog(@"提交日志");
    NSString *logFileUrl = [STIMKit updateLoadFile:[[STIMLocalLog sharedInstance] allLogData] WithMsgId:[STIMUUIDTools UUID] WithMsgType:STIMMessageType_File WithPathExtension:@"zip"];
    if (logFileUrl.length) {
        if (![logFileUrl stimDB_hasPrefixHttpHeader]) {
            logFileUrl = [NSString stringWithFormat:@"%@/%@", [[STIMKit sharedInstance] qimNav_InnerFileHttpHost], logFileUrl];
        }
...
- (void)sendFeedBackWithLogFileUrl:(NSString *)logFileUrl WithContent:(NSString *)content withUserInitiative:(BOOL)initiative {
    NSString *title = [NSString stringWithFormat:@"【IOS】来自：%@的反馈日志", [[STIMKit sharedInstance] getLastJid]];
    ...
    NSString *systemVersion = [[STIMKit sharedInstance] SystemVersion];
    NSString *appVersion = [[STIMKit sharedInstance] AppBuildVersion];
    ...
    NSString *eventName = [NSString stringWithFormat:@"反馈内容：%@\n平台：%@\n用户ID：%@\n导航地址: %@\n日志地址 : %@\n设备信息：%@\n设备系统版本：%@\nApp版本:%@", content, platName, [[STIMKit sharedInstance] getLastJid], [oldNavConfigUrlDict objectForKey:STIMNavUrlKey], logFileUrl, [[[STIMKit sharedInstance] deviceName] stringByReplacingOccurrencesOfString:@" " withString:@""], systemVersion, appVersion];
    ...
    NSData *requestData = [[STIMJSONSerializer sharedInstance] serializeObject:requestDic error:nil];
    ...
    [STIMHTTPClient sendRequest:request complete:^(STIMHTTPResponse *response) {
        if (response.code == 200) {
            STIMVerboseLog(@"提交日志成功");
            if (initiative == YES) {
                [[STIMLocalLog sharedInstance] deleteLocalLog];
                NSDictionary *responseDic = [[STIMJSONSerializer sharedInstance] deserializeObject:response.data error:nil];
